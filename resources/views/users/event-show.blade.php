        ...
        <div class="flex flex-col gap-8 ">
            ...
            <div class="flex flex-col gap-4">
                <h5
                    ...
                    <a href="{{ route('user.event') }}" class="text-primary">
                        event
                    </a>
                    <span class="text-textPrimary">»</span>
                    <span class="text-textPrimary">{{ $event->nama_event }}</span>
                </h5>

                <h1 class="capitalize font-semibold text-4xl mb-2 max-xl:text-3xl max-lg:mb-1">
                    {{ ucfirst($event->nama_event) }}
                </h1>
            </div>
            ...
            <div class="flex flex-col gap-8 max-md:gap-6">
                ...
                <p class="text-textSecondary font-light text-lg/9 max-md:text-base text-justify">{{ucfirst($event->deskripsi)  }}</p>

                <p class="text-textSecondary font-light text-lg/9 max-md:text-base">📅 Tanggal Mulai:
                    {{ Carbon\Carbon::parse($event->tanggal_mulai)->format('d F Y') }} </p>
                <p class="text-textSecondary font-light text-lg/9 max-md:text-base">📅 Tanggal Selesai:
                    {{ Carbon\Carbon::parse($event->tanggal_selesai)->format('d F Y') }} </p>
                <p class="text-textSecondary font-light text-lg/9 max-md:text-base">📍 Lokasi: {{ $event->lokasi }}</p>

                <p class="text-textSecondary font-light text-lg/9 max-md:text-base text-justify">
                    {{ $event->kalimat_penutup }}</p>
    ...
    <section class="max-w-screen-3xl mx-auto flex flex-col pb-20 px-4 sm:px-10">
        <div class="flex flex-col gap-12 max-md:gap-8">
            ...
            <div class="grid grid-cols-3 gap-10 max-lg:grid-cols-2 max-sm:grid-cols-1 max-md:gap-6">
                {{-- ? card looping --}}
                @foreach ($eventRekomendasi as $eventRekom)
                    @if ($eventRekom->nama_event != $event->nama_event)
                        <a href="{{ route('user.event.show', $eventRekom->id) }}"
                        ...
                        hover:-translate-y-2 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] cursor-pointer">
                            ...
                            <div class="w-full overflow-hidden rounded-xl">
                                <img src="{{ asset($eventRekom->image) }}" alt="{{ $eventRekom->nama_event }}"
                            ...
                            <div class="flex flex-col gap-4 max-md:gap-3">
                                ...
                                <div class="flex gap-3 items-center">
                                    ...
                                    <p
                                        class="text-textSecondary text-base capitalize leading-relaxed line-clamp-1 font-light">
                                        {{ Carbon\Carbon::parse($eventRekom->tanggal_mulai)->format('d F Y') }}
                                    </p>
