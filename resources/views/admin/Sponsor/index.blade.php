                    ...
                    <form action="{{ route('admin.sponsor.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <fieldset class="form-group">
                            <select class="form-select" id="basicSelect" name="status" required>
                                <option disabled selected hidden>Pilih Status Sponsor</option>
                        ...
                        <tbody>
                            @foreach ($sponsors as $sponsor)
                                <tr>
                                    ...
                                    <td class="text-truncate">{{ $sponsor->nama_perusahaan }}</td>
                                    <td>
                                        <img src="{{ asset($sponsor->image) }}" alt="{{ $sponsor->nama_perusahaan }}"
                                            width="80" class="img-thumbnail">
                                    </td>
                                    <td class="text-truncate {{ $sponsor->status == 'aktif' ? 'text-success' : 'text-danger' }}">{{ ucfirst($sponsor->status) }}</td>
                                    <td class="d-flex gap-2">
                                        ...
                                        <form action="{{ route('admin.sponsor.destroy', $sponsor->id) }}"
                                            ...
                                            <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                                data-bs-target="#confirmDeleteModal-{{ $sponsor->id }}">
                                                <i class="bi bi-trash"></i>
                                            </button>


                                            <!-- Modal alert hapus-->
                                            <div class="modal fade" id="confirmDeleteModal-{{ $sponsor->id }}"
